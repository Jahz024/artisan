    ...
    <AnimatePresence>
      {visible && node ? (
        <motion.div
          ...
          className="pointer-events-none absolute z-50 w-52 rounded-xl border border-slate-200 bg-white p-3 shadow-lg"
          style={{
            left: anchorX + 18,
            top: anchorY - 12,
          }}
          role="tooltip"
        >
          <p className="font-mono-accent text-xs font-semibold text-slate-900">
            {formatCourseCode(node.courseId)}
          </p>
          <p className="mt-1 text-[11px] leading-snug text-slate-600">{title}</p>
          <p className="mt-2 text-[10px] text-slate-500">
            {node.instructor ? `Prof. ${node.instructor}` : "Instructor TBD"}
          </p>
          <div className="mt-2 flex items-center justify-between gap-2">
            <Badge variant={node.status} className="!text-[9px]" />
            <span className="rounded-md bg-slate-100 px-2 py-0.5 font-mono-accent text-[10px] text-slate-700">
              Score {node.score}
            </span>
          </div>
        </motion.div>
