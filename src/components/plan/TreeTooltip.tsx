    ...
    <AnimatePresence>
      {visible && node ? (
        <motion.div
          ...
          className="pointer-events-none absolute z-50 w-52 rounded-xl border border-slate-600/60 bg-slate-950/92 p-3 shadow-2xl backdrop-blur-md"
          style={{
            left: anchorX + 18,
            top: anchorY - 12,
          }}
        ...
        >
          ...
          <p className="mt-1 text-[11px] leading-snug text-slate-300">{title}</p>
          <p className="mt-2 text-[10px] text-slate-500">
            {node.instructor ? `Prof. ${node.instructor}` : "Instructor TBD"}
          </p>
          <div className="mt-2 flex items-center justify-between gap-2">
            <Badge variant={node.status} className="!text-[9px]" />
            <span className="rounded-md bg-slate-800/90 px-2 py-0.5 font-mono-accent text-[10px] text-slate-200">
              Score {node.score}
            </span>
          </div>
        </motion.div>
